  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            _buildDetailRow('Predicted BG',
                widget.prediction.predicted_bg?.toStringAsFixed(2) ?? 'N/A'),
            _buildDetailRow(
                'Predicted Blood Glucose(mg/DL)',
                mMoiLToMgDl(widget.prediction.predicted_bg ?? 0)
                    .toStringAsFixed(2)),
            ...
            _buildDetailRow('Insulin Suggestion',
                widget.prediction.insulin_suggestion ?? 'N/A'),
            _buildDetailRow('Activity Suggestion',
                widget.prediction.activity_suggestion ?? 'N/A'),
            _buildDetailRow(
              'Predicted At',
            ...
            _buildDetailRow(
              'Next Check',
              formatCheckedAt(
                  widget.prediction.next_insulin_check_suggested_at),
            ),
            ...
            _buildDetailRow(
                'Blood Glucose',
                mMoiLToMgDl(widget.prediction.input_summary?.bg_mean ?? 0)
                    .toString()),
            _buildDetailRow(
                'BG Mean',
                widget.prediction.input_summary?.bg_mean?.toStringAsFixed(2) ??
                    'N/A'),
            _buildDetailRow(
                'Insulin Mean',
                widget.prediction.input_summary?.insulin_mean?.toString() ??
                    'N/A'),
            _buildDetailRow(
                'Carbs Mean',
                widget.prediction.input_summary?.carbs_mean?.toString() ??
                    'N/A'),
            _buildDetailRow(
                'Steps Mean',
                widget.prediction.input_summary?.steps_mean?.toString() ??
                    'N/A'),
            _buildDetailRow('HR Mean',
                widget.prediction.input_summary?.hr_mean?.toString() ?? 'N/A'),
            _buildDetailRow(
                'Cals Mean',
                widget.prediction.input_summary?.cals_mean?.toString() ??
                    'N/A'),
            _buildDetailRow(
                'Activity Encoded',
                widget.prediction.input_summary?.activity_encoded?.toString() ??
                    'N/A'),
            const SizedBox(height: 10),
            const Divider(),
